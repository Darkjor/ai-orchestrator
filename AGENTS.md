...
pip install -e .         # setup
pytest tests/ -v         # test
python -m aiorch.main triage   # run
```
...
- Run `pytest tests/ -v` before considering work done
...
**Never:**
- Commit code changes without updating `.ai/` or this file — enforced by the `ai-orch check` pre-commit hook
